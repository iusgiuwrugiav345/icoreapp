...
class Database:
    ...
    def create_email_verification(self, email, code=None, minutes_valid=10):
        """Creates a 6-digit verification code for an email and stores it."""
        try:
            conn = sqlite3.connect(config.DB_NAME_USERS)
            cursor = conn.cursor()
            ...
            code_val = code or f"{secrets.randbelow(1000000):06d}"
            cursor.execute("INSERT INTO auth_verifications (email, code, expires_at) VALUES (?, ?, ?)",
                           (email.strip().lower(), code_val, self._utc_iso(expires)))
            conn.commit()
            conn.close()
            return {"email": email.strip().lower(), "code": code_val, "expires_at": self._utc_iso(expires)}
        except Exception as e:
            print(f"Create email verification error: {e}")
            return None

    def verify_email_code(self, email, code):
        """Verify a code for an email. Marks used on success."""
        try:
            conn = sqlite3.connect(config.DB_NAME_USERS)
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, expires_at, used_at FROM auth_verifications WHERE email = ? AND code = ? ORDER BY id DESC LIMIT 1",
                (email.strip().lower(), str(code).strip())
            )
            row = cursor.fetchone()
            if not row:
                conn.close()
                return False
            vid, expires_at, used_at = row
            if used_at:
                conn.close()
                return False
            try:
                ...
                pass
            cursor.execute("UPDATE auth_verifications SET used_at = ? WHERE id = ?", (self._utc_iso(), vid))
            conn.commit()
            conn.close()
            return True
        except Exception as e:
