...
def get_webapp_url() -> str:
    return str(getattr(config, "WEBAPP_URL", "") or "").strip()
...
def get_main_keyboard():
    ...
    ) if webapp_url else KeyboardButton(text="Catalog")
    keyboard = ReplyKeyboardMarkup(
        keyboard=[[
            KeyboardButton(text="❤️ Поддержать донатом"),
...
def get_donate_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💳 Mastercard", callback_data="donate_mastercard")],
        [InlineKeyboardButton(text="💠 Visa", callback_data="donate_visa")],
        [InlineKeyboardButton(text="🪙 Crypto", callback_data="donate_crypto")],
        [InlineKeyboardButton(text="✖ Закрыть", callback_data="donate_close")]
    ])


def get_donate_detail_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
...
def _content_type_label(content_type: str) -> str:
    return "Приложение" if str(content_type or "").strip().lower() in {"app", "apps", "application"} else "Игра"


async def send_catalog_item_to_chat(chat_id: int, user_id: int, *, game_name: str | None = None, item_id: int | None = None):
    row = db.get_game_by_id(item_id) if item_id else db.get_game_by_name(game_name or "")
    resolved_item_id = item_id

    if not row:
        await bot.send_message(chat_id, "❌ Игра или приложение не найдено")
        return False, None

    name, url, content_type = row[0], row[1], row[2]
    if not resolved_item_id:
        resolved_item_id = db.get_game_id_by_name(name)
    ...
    try:
        ...
        if url.startswith("http"):
            try:
                await bot.send_document(
                    chat_id,
                    url,
                    caption=f"{_content_type_label(content_type)}: {name}\n{rating_line}",
                    reply_markup=get_game_keyboard(name, resolved_item_id)
                )
                ...
                try:
                    ...
                    await bot.send_message(
                        chat_id,
                        f"{_content_type_label(content_type)}: {name}\n{rating_line}\n📥 Ссылка для скачивания: {url}",
                        reply_markup=get_game_keyboard(name, resolved_item_id)
                    )
                    download_success = True
                except Exception:
                    await bot.send_message(chat_id, "❌ Извините, файл в данный момент недоступен.")
                    return False, name
        ...
        if download_success:
            ...
            if recommendations:
                ...
                await bot.send_message(chat_id, "🎯 *Вам также может понравиться:*", parse_mode="Markdown", reply_markup=rec_keyboard)
            return True, name
    ...
    try:
        ...
        await message.answer_photo(
            ...
            caption=("👋 Привет! Я бот для скачивания .IPA файлов\n\n"
                     ...
                     "❤️ /buy - поддержать проект донатом\n"
                     "💻 @iCoreapp - наш канал"
                    ),
...
async def catalog(message: types.Message, state: FSMContext):
    ...
    url = get_webapp_url()
    if not url:
        return await message.answer("Catalog is not configured. Set WEBAPP_URL in config.py")

    keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Open Catalog", web_app=types.WebAppInfo(url=url))]])
    await message.answer("Open the mini app catalog:", reply_markup=keyboard)
...
async def buy(message: types.Message, state: FSMContext):
    ...
    text = (
        ...
        "Если хотите помочь развитию бота и mini app, выберите удобный способ ниже.\n\n"
        "<i>Все реквизиты открываются по кнопке.</i>"
    )
    await message.answer(text, parse_mode="HTML", reply_markup=get_donate_menu_keyboard())


@dp.callback_query_handler(lambda c: c.data == "donate_back", state="*")
async def donate_back(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(text, parse_mode="HTML", reply_markup=get_donate_menu_keyboard())
    await callback_query.answer()


@dp.callback_query_handler(lambda c: c.data == "donate_mastercard", state="*")
async def donate_mastercard(callback_query: types.CallbackQuery, state: FSMContext):
    await state.finish()
    text = (
        "<b>💳 Mastercard</b>\n\n"
        "<b>Номер:</b>\n"
        "<code>5477 3300 4095 4475</code>\n\n"
        "<b>Владелец:</b>\n"
        "<code>Timur Mukumov</code>\n\n"
        "Спасибо за поддержку проекта."
    )
    await callback_query.message.edit_text(text, parse_mode="HTML", reply_markup=get_donate_detail_keyboard())
    await callback_query.answer()


@dp.callback_query_handler(lambda c: c.data == "donate_visa", state="*")
async def donate_visa(callback_query: types.CallbackQuery, state: FSMContext):
    await state.finish()
    text = (
        "<b>💠 Visa</b>\n\n"
        "<b>Номер:</b>\n"
        "<code>4916 9907 0932 2678</code>\n\n"
        "<b>Владелец:</b>\n"
        "<code>Timur Mukumov</code>\n\n"
        "Спасибо за поддержку проекта."
    )
...
@dp.callback_query_handler(lambda c: c.data == "donate_crypto", state="*")
async def donate_crypto(callback_query: types.CallbackQuery, state: FSMContext):
    await state.finish()
    text = (
        "<b>🪙 Crypto</b>\n\n"
        "<b>ETH</b>\n"
        "<code>0xf7d765952777F091C124e735D9C373A37Cbb59B3</code>\n\n"
        "<b>BTC</b>\n"
        "<code>bc1qrtuw5f047s56xtf5ptgxz6kk045e87kxudg2su</code>\n\n"
        "<b>SOL</b>\n"
        "<code>XkHR6mL2xR3oCwQsA5LQUCCHsd8hCxEkijCL4kAUh3d</code>\n\n"
        "<b>TRON</b>\n"
        "<code>TVHfb4XXG3wdpLxjb8PrLsUUKPbpqjfzew</code>\n\n"
        "Спасибо за поддержку проекта."
    )
    await callback_query.message.edit_text(text, parse_mode="HTML", reply_markup=get_donate_detail_keyboard())
    await callback_query.answer()
...
@dp.callback_query_handler(lambda c: c.data == "donate_close", state="*")
async def donate_close(callback_query: types.CallbackQuery, state: FSMContext):
    await state.finish()
    await callback_query.message.edit_text(
        "<b>Окно доната закрыто.</b>\n\nЕсли захотите поддержать проект позже — нажмите кнопку снова.",
        parse_mode="HTML"
    )
    await callback_query.answer()
...
async def handle_recommendation_download(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await send_catalog_item_to_chat(
...
async def about_info_callback(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        "👨‍💻 Разработчик — @hydrauk.\n"
        "🛡️ Файлы безопасны.\n"
        "❤️ Поддержка донатом помогает развитию проекта.\n"
...
async def process_suggestion(message: types.Message, state: FSMContext):
    menu_buttons = ["❤️ Поддержать донатом", "💎 Купить подписку", "❣️ Помощь", "№️ О Нас", "💡 Предложить игру"]
...
@dp.message_handler(commands=['seticon'], state="*")
async def set_icon_admin(message: types.Message, state: FSMContext):
    await state.finish()
    if message.from_user.id not in config.ADMINS:
        return
    ...
    if len(parts) != 2 or not parts[0].isdigit():
        return await message.answer("Использование: /seticon <item_id> <https://url>")

    item_id = int(parts[0])
    icon_url = parts[1].strip()
    if not icon_url.startswith(("http://", "https://")):
        return await message.answer("❌ Иконка должна быть прямой http/https ссылкой.")
    if not db.game_exists_by_id(item_id):
        return await message.answer("❌ Такой записи нет.")

    if db.set_game_icon(item_id, icon_url):
        await message.answer(f"✅ Иконка обновлена для item_id `{item_id}`", parse_mode="Markdown")
    else:
        await message.answer("❌ Не удалось сохранить иконку.")


@dp.message_handler(commands=['setcover'], state="*")
async def set_cover_admin(message: types.Message, state: FSMContext):
    await state.finish()
    if message.from_user.id not in config.ADMINS:
        return
    ...
    parts = message.get_args().split(maxsplit=1)
    if len(parts) != 2 or not parts[0].isdigit():
        return await message.answer("Использование: /setcover <item_id> <https://url>")

    item_id = int(parts[0])
    cover_url = parts[1].strip()
    if not cover_url.startswith(("http://", "https://")):
        return await message.answer("❌ Cover должен быть прямой http/https ссылкой.")
    if not db.game_exists_by_id(item_id):
        return await message.answer("❌ Такой записи нет.")

    if db.set_game_cover(item_id, cover_url):
        await message.answer(f"✅ Cover обновлен для item_id `{item_id}`", parse_mode="Markdown")
    else:
        await message.answer("❌ Не удалось сохранить cover.")
...
async def handle_webapp_data(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        return await message.answer("❌ Не удалось обработать команду из mini app.")
    ...
    try:
        ...
        return await message.answer("❌ Некорректный ID приложения.")

    await message.answer("📦 Отправляю файл или ссылку в этот чат...")
    await send_catalog_item_to_chat(message.chat.id, message.from_user.id, item_id=item_id)
...
async def handle_game_query(message: types.Message, state: FSMContext):
    ...
    if message.text in {"❤️ Поддержать донатом", "💎 Купить подписку"}:
    ...
    if message.text == "Catalog":
        ...
        if not url:
            return await message.answer("Catalog is not configured. Set WEBAPP_URL in config.py")
        keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Open Catalog", web_app=types.WebAppInfo(url=url))]])
        return await message.answer("Open the mini app catalog:", reply_markup=keyboard)
